...
exports.getCOPOMappings = exports.createCOPOMapping = exports.getCourseOutcomesByCourse = exports.getProgramOutcomesByCourse = exports.getAllProgramOutcomes = exports.createProgramOutcome = void 0;
...
const createBulkCOPOMappingSchema = zod_1.default.object({
    mappings: zod_1.default.array(createCOPOMappingSchema)
});
const copoMappingSchema = zod_1.default.object({
    coId: zod_1.default.number().int().positive(),
    poId: zod_1.default.number().int().positive(),
    weightage: zod_1.default.enum(['0', '0.5', '1', '1.5', '2', '2.5', '3']).transform(val => parseFloat(val)),
});
// Schema for bulk operations
const bulkCOPOMappingSchema = zod_1.default.object({
    mappings: zod_1.default.array(copoMappingSchema)
});
const createCOPOMapping = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const existingMappings = yield prisma.cO_PO_Mapping.findMany({
            where: {
        ...
        const existingPairsMap = new Map(existingMappings.map(mapping => [`${mapping.coId}-${mapping.poId}`, mapping.id]));
        // Track operations for response
        let createdCount = 0;
        let updatedCount = 0;
        // Use upsert for each mapping in a transaction
        const results = yield prisma.$transaction(mappingsToProcess.map(mapping => {
