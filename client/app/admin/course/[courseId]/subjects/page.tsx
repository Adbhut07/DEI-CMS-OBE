...
export default function SubjectsPage() {
  ...
  const handleSaveMapping = async (mapping: { subjectId: number; semester: number; batchId: number }) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/api/v1/course-subject-mapping/map`, {
        method: 'POST',
        credentials: 'include',
        headers: {
  ...
  const handleDeleteMapping = async (id: number) => {
    if (window.confirm('Are you sure you want to remove this subject from the course?')) {
      setIsLoading(true);
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/api/v1/course-subject-mapping/unmap/${id}`, {
          method: 'DELETE',
          credentials: 'include',
        });
