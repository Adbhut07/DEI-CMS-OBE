...
export default function SubjectsPage() {
  ...
  const fetchSubjects = async () => {
    try {
      setLoading(true)
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/api/v1/subjects/getAllSubjects`, {
        credentials: "include",
      })
      if (!response.ok) {
  ...
  const fetchUnits = async (subjectId: number) => {
    try {
      setLoading(true)
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/api/v1/units/getAllUnits/${subjectId}`, {
        credentials: "include",
      })
      if (!response.ok) {
  ...
  const createSubject = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/api/v1/subjects/create`, {
        credentials: "include",
        method: "POST",
        headers: {
  ...
  const createUnits = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/api/v1/units/bulkCreate`, {
        credentials: "include",
        method: "POST",
        headers: {
          "Content-Type": "application/json",
