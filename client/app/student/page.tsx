...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    async function fetchStudentDetails() {
      try {
        setLoading(true)
        const response = await fetch("http://localhost:8000/api/v1/student/details", {
          credentials: "include",
        })
  ...
  const fetchSemesterInfo = async () => {
    try {
      const response = await fetch("http://localhost:8000/api/v1/semester/student", {
        credentials: "include",
      })
  ...
  const fetchStudentMarks = async (semester: number) => {
    try {
      ...
      const response = await fetch("http://localhost:8000/api/v1/student/marks", {
        method: "POST",
        headers: {
